...
const NON_DIRECT_ITEM_EVIDENCE_RE = /\brepresentative\s+(?:sample|evidence|spot[- ]?check)\b|\b(?:item(?:'s)?\s+own\s+)?(?:specific\s+)?citation\s+(?:was\s+)?not\s+independently\s+re[- ]?read\b|\bnot\s+independently\s+(?:re[- ]?read|read|verified|checked)\b|\bcitation\s+(?:is\s+|was\s+)?used\s+as[- ]is\b|\bfollows?\s+(?:the\s+)?same\s+verified\s+(?:batch\s+)?pattern\b|\bsame\s+(?:verified\s+)?citation\s+pattern\b|\bnot\s+(?:item|claim)[- ]specific\b|\b(?:item|claim)[- ]specific\b.{0,80}\b(?:still\s+)?(?:needs?|requires?)\b.{0,32}\b(?:provenance|verification|re[- ]?read|evidence|source(?:\s+support)?)\b|\b(?:item|claim)[- ]specific\b.{0,48}\b(?:provenance|verification|re[- ]?read|evidence|source(?:\s+support)?)\b.{0,24}\b(?:(?:is|remains?)\s+)?(?:still\s+)?(?:required|needed|missing|unresolved|incomplete)\b|\b(?:échantillon|echantillon)\s+repr(?:é|e)sentatif(?:ve)?\b|\b(?:citation|référence|reference|preuve|provenance)\b.{0,48}\b(?:non|pas)\s+(?:relue?|vérifi(?:é|e)e?|verifi(?:é|e)e?|contrôl(?:é|e)e?|controlee?)\s+indépendamment\b|\b(?:preuve|evidence|provenance|citation|référence|reference)\b.{0,32}\bnon\s+sp(?:é|e)cifique\b.{0,24}\b(?:item|élément|element|point|question|claim)\b|\b(?:item|élément|element|point|question|claim)\b.{0,24}\bnon\s+sp(?:é|e)cifique\b|\b(?:provenance|preuve|evidence|citation|référence|reference)\b.{0,32}\bsp(?:é|e)cifique\s+(?:à|a)\s+(?:l['’]?item|l['’]?(?:élément|element)|la\s+question|au\s+point|au\s+claim)\b.{0,48}\b(?:reste|restent|est|sont)\s+(?:à|a)\s+(?:confirmer|vérifier|verifier|documenter|tracer|relire)\b|\b(?:item|élément|element|point|question|claim)[- ]sp(?:é|e)cifique\b.{0,64}\b(?:reste|restent|est|sont)\s+(?:à|a)\s+(?:confirmer|vérifier|verifier|documenter|tracer|relire)\b/i;
